...
# Test region
xmin, ymin, xmax, ymax = 3343824,1882606,3363178,1905273

pdo_shp = gpd.read_file(pdo_path)#.cx[xmin:xmax, ymin:ymax]
vineyards_shp = gpd.read_file('../data/vineyards_europe_lau.shp')#.cx[xmin:xmax, ymin:ymax]
pdo_name_map = pd.read_csv('prepared_data/candiago_2022.csv')
...
##Calculate vineyard area per NUTS
# eurostat_vineyards = gpd.overlay(eurostat_shp, vineyards_shp, how = 'intersection', keep_geom_type=True)
# eurostat_vineyards = (
#     eurostat_vineyards.loc[eurostat_vineyards.geometry.area >= 100000]
#     .dissolve(by = 'NUTS_ID')
#     .reset_index()
#     .drop(['LAUid'], axis = 1)
# )
# eurostat_vineyards['Vin_Area_NUTS'] = eurostat_vineyards.geometry.area
# eurostat_vineyards.drop(['geometry'], axis = 1, inplace = True)

var_name = "GEWUERZTRAMINER"

##Select pdos and nuts that contain variety
pdos_var = pdo_name_map.loc[pdo_name_map['Prime Name'] == var_name, 'PDOid'].unique()
...
if pdo_vineyards_nuts.shape[0] > 0:
    ...
    pdo_cult_area['Variety'] = var_name
    ##
